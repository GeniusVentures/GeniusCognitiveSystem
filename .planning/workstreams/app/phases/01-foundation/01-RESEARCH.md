...
- **D-26:** **Publish/subscribe FFI; protobuf wire format (2026-08-22).** Reverses D-24's "FFI/JSON only" transport: (1) **push, not pull** — all data flows native→Dart on the D-05 callback stream (messages, room-list/membership, readiness); no `gcs_joined_topics` pull call; (2) **protobuf is the FFI wire format** — the message/event contract is `gcs_chat.proto` (compiled via `add_proto_library` + `find_package(Protobuf CONFIG REQUIRED)`, already wired in `cmake/CommonBuildParameters.cmake`), the C++ half is the generated `ChatMessageState` reused directly (no `chat_message_state.hpp.jinja2` + nlohmann_json), append-only `.proto` from day one. Full text in 01-CONTEXT.md.
...
- FFI transport mechanism for the generated C++ Cubit-state halves (per ai-boss frontend-templates phase 07 discussion: Claude's discretion; must be C++17-clean — see "Composite C++ Half" below). **Resolution direction locked by D-24, amended by D-26 (2026-08-22): protobuf is Phase 1's importer (superseding FFI/JSON); the store surface is contract-agnostic (their D-04) so the protobuf wire types never leak into it.**
...
- Scaffold's `templates/cpp/` is an **empty placeholder** — its README explicitly says C++ template content is owned by the consuming repo (verified: `src/app/scaffold/templates/cpp/README.md`). Under D-26 the C++ half of the message contract is NOT a Jinja2-authored C++ struct — it is the **protobuf-generated `ChatMessageState`** in `src/proto/gcs_chat.proto` (compiled via `add_proto_library`). Only the Dart widget half is Jinja2-generated from `_vars.json`; the C++ half is the reused proto type.
- The FFI transport mechanism is fixed by D-26 (2026-08-22): the wire format is **protobuf**, and the message/event payload crossing `Dart_PostCObject` is serialized `gcs.chat.GcsEvent` bytes (not a JSON string). The generated C++ Cubit-state half is therefore the **protobuf-generated `gcs::chat::ChatMessageState` type reused directly** — no `chat_message_state.hpp.jinja2` + `nlohmann_json` struct. The `.proto` is compiled via the existing `add_proto_library` helper (`build/cmake/functions.cmake:104`) + `find_package(Protobuf CONFIG REQUIRED)` (already wired in `cmake/CommonBuildParameters.cmake:36-61`; three `.proto` files already exist in `GNUS-NEO-SWARM/src/proto/` as the pattern). The Dart side decodes the same proto (`GcsEvent.fromBuffer`) into generated Dart classes. One `.proto`, two generated endpoints (C++ `.pb.h` + Dart `.pb.dart`) — no translate layer.
- **Phase 1 scoping (LOCKED by D-24, amended by D-26, 2026-08-22):** the C++ half is generated this phase, minimal — the authoritative message record (`id`, `room_topic`, `role`, `state`, `text`, `timestamp` — append-only optional adds later) is `ChatMessageState` in `src/proto/gcs_chat.proto` (D-26). The `.proto` is append-only from day one (ai-boss D-06/D-07). This satisfies "always generate both halves" (their D-02) without building the full pluggable-importer framework (ai-boss phase 07's deliverable, not ours) — but the store stays contract-agnostic with the importer seam from day one, so the protobuf transport (Phase 1's importer, per D-26) never leaks into the store's public surface and later importers (OpenAPI, CRDT schema) slot in without touching it.
...
| A8 | The message event is serialized `gcs.chat.GcsEvent` protobuf bytes across FFI, decoded by generated Dart classes mirroring the generated C++ proto type (one `.proto`, two endpoints, `add_proto_library` on the C++ side) | Composite C++ Half | Medium — the C++ half is the protobuf-generated `ChatMessageState` reused directly per D-26 (protobuf is Phase 1's importer, superseding D-24's "FFI/JSON only"); the full pluggable-importer model belongs to ai-boss phase 07. Both-halves generation is LOCKED (D-24, amended D-26) — not deferrable. |
...
4. **Does the FFI session API carry room-list/readiness events in Phase 1, or only message events?** — **RESOLVED (2026-08-22) by D-26 (push, not pull):**
   - Room-list / membership / readiness are **pushed events** on the D-05 callback stream (like messages), NOT a pull call. `gcs_joined_topics(handle)` is rejected. The session pre-joins a small smoke-topic set (≥2 topics, e.g. `gcs/chat/smoke-test` + one more) at init and pushes the room list to `RailCubit`; switching is demonstrable (D-21). The room-list data source stays behind the SessionCubit projection so Phase 2 swaps in real spaces/rooms (CRDT schema) without touching the rail widget. No spaces/rooms data model is designed this phase (Phase 2 + deferred CRDT schema).

5. **Where do the generated C++ Cubit-state halves live and how minimal are they in Phase 1?** — **RESOLVED (2026-08-22) by D-26 (amending D-24):** The C++ half is the protobuf-generated `ChatMessageState` in `src/proto/gcs_chat.proto`, compiled via `add_proto_library` and reused directly as store state — no `chat_message_state.hpp.jinja2` + `nlohmann_json` struct (D-24's JSON transport is superseded by D-26's protobuf). Establishes the both-halves invariant cheaply and keeps the `.proto` single-sourced. Merged ai-boss 07 constraints: the store is contract-agnostic with a pluggable importer seam (their D-04 — protobuf is Phase 1's importer, never leaked into the store surface), reuses contract types directly (their D-05), append-only `.proto` from day one (their D-06/D-07).
...
- [ ] `src/app/test/theme_registration_test.dart`, `chat_shell_test.dart`, `generated/chat_composites_test.dart` — OWNED by 01-11 Task 4 (replaces the stock `widget_test.dart`, which references a nonexistent `MyApp` and would fail analyze today)
