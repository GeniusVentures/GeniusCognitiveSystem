...
### Locked Decisions (UI strand, D-10..D-22)
...
- **Phase 1 scoping recommendation:** the C++ half generation can be minimal this phase — the message record struct (`id`, `room_topic`, `role`, `state`, `text`, `timestamp` — all append-only optional adds later) emitted from one `templates/cpp/chat_message_state.hpp.jinja2` driven by the same `_vars.json` as the bubble. This satisfies "always generate both halves" without building the full pluggable-importer framework (which is ai-boss phase 07's deliverable, not ours). Flag as planner discretion; the FFI JSON transport is the only importer Phase 1 exercises.
...
| A8 | The message event JSON posted across FFI can be decoded by generated Dart state classes that mirror a generated C++ struct (one schema, two endpoints, nlohmann_json on the C++ side) | Composite C++ Half | Medium — the C++ half's template shape is Phase 1-minimal by recommendation; the full pluggable-importer model belongs to ai-boss phase 07. If the user wants the C++ half deferred entirely, only the `templates/cpp/` task drops out. |
...
2. **CRDT round-trip needs GcsGlobalDb accessors (Put/Get/AddBroadcastTopic) that don't exist yet.** — **RESOLVED (2026-08-15):** Plan 01-01 adds the four pass-through accessors to `GcsGlobalDb` on a GNUS-NEO-SWARM feature branch (`feature/gcs-globaldb-passthrough`, stacked on `feature/app-restructure`).
...
5. **Where do the generated C++ Cubit-state halves live and how minimal are they in Phase 1?**
   - What we know: ai-boss phase-07 locks "always generate both halves" and "FFI transport = Claude's discretion, C++17-clean"; scaffold's `templates/cpp/` is an explicitly empty consumer-owned placeholder.
   - What's unclear: Whether Phase 1 generates a C++ message-record struct from `_vars.json` (recommended, one template) or defers C++-half generation to a later phase and hand-writes the one record struct.
   - Recommendation: Generate it (one `chat_message_state.hpp.jinja2` driven by the bubble's vars) — it establishes the both-halves invariant cheaply and keeps the JSON schema single-sourced. Flag for user confirmation at plan review (A8).
