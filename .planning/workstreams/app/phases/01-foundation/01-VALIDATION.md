...
| 01-03-01 | 03 | 3 | CORE-05 | — | `gcs_chat.proto` + `gcs_proto` target via add_proto_library | static / grep | `test -f src/proto/gcs_chat.proto && grep -q "package gcs.chat" src/proto/gcs_chat.proto && grep -q "add_proto_library(gcs_proto" src/proto/CMakeLists.txt && grep -q "add_subdirectory(proto)" src/CMakeLists.txt` | ❌ W0 | ⬜ pending |
| 01-03-02 | 03 | 3 | CORE-05 | T-01-05 (FFI export surface) | Single opaque-handle C API declared; `_WIN32` dllexport macro block is the ONLY OS preprocessor guard | static / grep | `grep -cE "GCS_FFI_API.*gcs_(init\|shutdown\|join_topic\|publish\|on_message\|string_free)" src/ffi/gcs_core.h \| grep -q "^6$" && ! grep -q "GCS_ERROR_NOT_IMPLEMENTED" src/ffi/gcs_core.h` | ❌ W0 | ⬜ pending |
| 01-03-03 | 03 | 3 | CORE-05 | T-01-02 (heap mismatch) / T-01-04 (double-shutdown) | All six `gcs_*` symbols exported from `libgcs_ffi.{dylib,so,dll}`; every function takes the global mutex; protobuf serialize + pushed RoomList/Readiness | build + nm | `cmake --build build/OSX/Debug -j 2>&1 \| tee /tmp/gcs_ffi_build.log \| tail -5 ; test -f build/OSX/Debug/gcs_src/ffi/libgcs_ffi.dylib && nm -gU build/OSX/Debug/gcs_src/ffi/libgcs_ffi.dylib \| grep -cE "_gcs_(init\|shutdown\|join_topic\|publish\|on_message\|string_free)" \| grep -q "^6$"` | ❌ W0 | ⬜ pending |
...
| 01-04-03 | 04 | 4 | CORE-05 | T-01-01 (FFI boundary null/malformed input) / T-01-03 (use-after-free) | FFI surface smoke; `gcs_init` returns `nullptr` gracefully (no crash, no exception) when GeniusSDK node unavailable; argument validation + shutdown-safe-on-null tests do not require a live node; **no `gcs_on_message` live-handle tests in this plan** | unit (gtest) | `grep -c "sleep_for\|sleep_until" test/test_gcs_ffi.cpp \| grep -q "^0$" && cmake --build build/OSX/Debug -j 2>&1 \| tail -5 \| grep -E "error\|FAILED" ; ctest --test-dir build/OSX/Debug -R test_gcs_ffi --output-on-failure 2>&1 \| tail -15 \| grep -E "Passed\|Failed\|tests passed"` | ❌ W0 | ⬜ pending |
...
| 01-05-03 | 05 | 5 | CORE-05 | — | ffigen bindings + protobuf Dart bindings generated against the real src/ffi/gcs_core.h + src/proto/gcs_chat.proto | codegen + grep | `test -f src/app/ffigen.yaml && grep -q "gcs_core.h" src/app/ffigen.yaml && test -f src/app/lib/gcs_bindings_generated.dart && grep -c "gcs_init\|gcs_shutdown\|gcs_on_message" src/app/lib/gcs_bindings_generated.dart \| grep -q "^[3-9]\|^[1-9][0-9]" && test -f src/app/lib/generated/proto/gcs_chat.pb.dart && grep -q "GcsEvent" src/app/lib/generated/proto/gcs_chat.pb.dart` | ❌ W0 | ⬜ pending |
| 01-05-04 | 05 | 5 | CORE-05 | — | Dart smoke: `gcs_init` → `gcs_on_message` → pushed RoomList/Readiness + `gcs_publish` echo decoded as ChatMessageState (skip-on-null-init only) | manual / flutter test | `cd src/app && flutter test test/gcs_native_port_smoke_test.dart` | ❌ W0 | ⬜ pending (human-verify) |
...
|----------|-------------|------------|-------------------|
| Dart protobuf NativePort round-trip smoke (`gcs_init` → `gcs_on_message` → pushed RoomList/Readiness + `gcs_publish` echo decoded as ChatMessageState) | CORE-05 success criterion 3 | Requires a Flutter runtime + a loaded dylib; cannot be driven from ctest. `skip:` only for the live-node init-success path when GeniusSDK is unavailable in the test env. | `cd src/app && flutter test test/gcs_native_port_smoke_test.dart` — expect "+1 passed" or skip-with-note; Plan 01-05 Task 4 human-verify checkpoint |
